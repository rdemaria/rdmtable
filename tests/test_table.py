...
def test_column_selection():
    ...
    assert t.cols[["betx", "bety"]].betx[0] == t.betx[0]
    assert t.cols["sqrt(betx)>3 sqrt(bety)"]['sqrt(betx)>3'][0]==(np.sqrt(data['betx'][0])>3)


def test_row_selection():
    assert t.rows[1].betx==data['betx'][1]
    assert t.rows[[0, 2]].betx[1]==data['betx'][2]
    assert t.rows[t.s > 1].betx[1]==data['betx'][t.s > 1][1]
